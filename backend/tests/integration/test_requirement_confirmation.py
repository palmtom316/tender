...
def test_extract_veto_requirement():
    ...
    results = asyncio.get_event_loop().run_until_complete(extract_requirements(sections))
...
def test_extract_scoring_from_table():
    ...
    results = asyncio.get_event_loop().run_until_complete(extract_scoring_criteria(tables))
