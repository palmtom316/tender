...
def create_project(payload: ProjectCreate, conn: Connection = Depends(get_db_conn)) -> ProjectOut:
    project = _repo.create(conn, name=payload.name.strip())
    return ProjectOut(id=project.id, name=project.name)


@router.get("/projects", response_model=list[ProjectOut])
def list_projects(conn: Connection = Depends(get_db_conn)) -> list[ProjectOut]:
