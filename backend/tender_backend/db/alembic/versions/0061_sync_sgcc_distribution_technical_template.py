"""sync sgcc distribution technical template and prune legacy rows
...
def _code_list_sql() -> str:
    return ", ".join(f"'{code}'" for code, _title in _ITEMS)
...
def _cleanup_sql() -> str:
    ...
    WHERE package_id IN (
      ...
      AND item_code NOT IN ({_code_list_sql()});
    """
...
def downgrade() -> None:
    # Do not recreate stale historical rows on downgrade. 0045 remains the
    # historical seed; this migration is a forward data correction.
    pass
