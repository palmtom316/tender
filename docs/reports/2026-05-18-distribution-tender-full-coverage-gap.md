...
> **审阅方式:** 只读代码盘点；2026-05-18 结合现有代码二次核实后修订
...
| 2 | 监理人员执业合规承诺函 | ✅ | ⚠️ 通用生成 fallback | — | **缺承诺函专用模板与签章变量填充** |
| 3 | 工期响应 | ✅ | ⚠️ 通用生成 fallback | — | **缺工期响应表/里程碑约束专用生成** |
| 4 | 资质情况 | ✅ | ⚠️ companybase 拉取 + 通用生成 | — | 资质类型枚举弱、无证书自动筛选与组装闭环 |
| 5 | 业绩情况（5.1-5.4）| ✅ | ⚠️ companybase 拉取 + 通用生成 | — | 缺 kV/容量/回路自动筛选、业绩证明 DOCX 组装 |
| 6 | 项目团队情况 | ✅ | ✅ 短策略 (`registry.py:303`) | ❌ | **未启用 longform**，无人员证书自动组装 |
| 7 | 其他资格条件 | ✅ | ⚠️ 通用生成 fallback | — | **缺其他资格条件专用容器与资料映射** |
...
| 11 | 服务承诺 | ✅ | ⚠️ 通用生成 fallback | ❌ | **缺服务承诺专用策略**（与 8.12 存重叠风险） |
| 12 | 技术评分支撑材料 | ✅ | ✅ 短策略 (`registry.py:396`) | ❌ | 仅占位，无评分点扫描+材料索引 |
| 13 | 技术规范书响应 | ✅ | ✅ 短策略 (`registry.py:409`) | ❌ | 仅占位，无逐条响应生成 |
| 14 | 履约评价证明材料 | ✅ | ⚠️ companybase/evidence_asset + 通用生成 | — | 缺履约评价证明材料筛选、附件注入与版式闭环 |
| 15 | 其他 | ✅ | ⚠️ 通用生成 fallback | — | 缺补充材料容器与资料绑定规则 |
| 16 | 履约承诺函 | ✅ | ⚠️ 通用生成 fallback | — | **缺履约承诺函专用模板与签章变量填充** |

**核心结论:** 16 章中
- **5 章 longform 完备**（8、9、10.1/10.2/10.3，合计约 215 页）
- **3 章短策略占位**（6、12、13）
- **8 章缺专用生成与交付语义**（2、3、7、11、14、15、16；4、5 主要依赖 companybase/通用生成），当前只能依赖 AI gateway 或 deterministic fallback 生成普通正文，不能证明具备正式标书所需的模板、附件、签章、索引和校验闭环。

**核实后校准口径:** 技术章节生成主流程并非只允许 5 章输出；`TechnicalBidWriter.generate_chapter` 对非 longform 章节仍可走 AI gateway 或 deterministic fallback。因此本报告中的“缺口”不应理解为“代码完全无法生成任何文本”，而应理解为“缺少面向该章业务语义的专用模板、结构化资料映射、附件/DOCX 组装和质量门禁”。
...
| 项 | 现状 | 缺口 |
|---|---|---|
| 多章节 e2e 入口 | `scripts/run_longform_multi_chapter_acceptance.py` 默认 5 章（8、9、10.1、10.2、10.3）| **不能证明完整 16 章 DOCX 可交付**；缺 1-7/11-16 章生成、资料绑定、导出和验收的一体化入口 |
| DOCX 组装 | `docx_exporter` 可导出数据库中已有 `chapter_draft`，并支持偏差表、人员表、设备表、图表注入 | 缺承诺函、工期响应、资质/业绩附件、评分支撑、规范响应等章节的专用版式与附件组装语义；仅拼已有草稿不能等同于完整标书闭环 |
| 暗标合规 | 图表链路具备脱敏/扫描能力，longform 章节有较明确的禁用项与质量门禁 | 1-7/11-16 章未纳入同等级的章节级暗标校验与阻断；人员、业绩、证书、履约评价附件仍需统一脱敏策略 |
...
| **P0** | 建立完整 16 章交付验收入口：逐章检查草稿、附件绑定、签章/承诺函模板、暗标风险、页数/图表/表格占位和导出结果 | "能否证明完整交付" |
...
- 技术章节生成：`backend/tender_backend/services/technical_bid_writer.py:58-148`（longform / AI gateway / deterministic fallback 分支）
- DOCX 导出：`backend/tender_backend/services/export_service/docx_exporter.py:300-379`（导出已有 chapter_draft、偏差表、人员表、设备表、图表注入）
...
| v1.1 | 2026-05-18 | 结合现有代码复核后修订：收紧“完全无生成逻辑”“不能产出完整 DOCX”“暗标过滤”等表述，明确通用生成 fallback 与专用交付闭环之间的边界，并补充完整 16 章交付验收入口建议。 |
